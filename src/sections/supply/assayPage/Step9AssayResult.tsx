    ...
    return (
      ...
      >
        {(formikProps) => (
          <Form>
            <Stack spacing="6">
              <Select placeholder="Assay Result" isRequired onChange={(v) => formikProps.setFieldValue('assayResult', v.target.value)}>
                {['yes', 'no'].map((assayResult, i) =>
                  <option style={{ textTransform: 'capitalize' }} key={i} value={assayResult}>
                    {assayResult}
                  </option>)}
              </Select>

              <ImageUpload
                name={'assayReport'} label={'Assay Report'}
                onChange={(url: string) => formikProps.setFieldValue('assayReport', url)}
                filePath='assayReport'
              />
...
export default Step9AssayResult;